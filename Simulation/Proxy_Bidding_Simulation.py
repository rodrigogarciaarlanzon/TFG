...
def sim_reserv(n, reserve_price: list, min_increment, simulations=1000):
    results = []
...
def ejecutar_simulaciones(n):
    Reserv_price = np.linspace(0, 1, 20)          # rango de precios de reserva
    Min_increment = np.linspace(0, 0.3, 20)     # rango de incrementos mínimos

    results_reserv = sim_reserv(n, Reserv_price, min_increment=0.05) #determinamos el incremento mínimo de puja
    results_increment = sim_increment(n, min_increment=Min_increment, reserve_price=0.2)
